...
export function UserStatusDisplay({ status, nowMs, inline = false, compact = false, id }: { id?: string; status?: UserStatus | null; nowMs: number; inline?: boolean; compact?: boolean }) {
  const active = activeUserStatus(status, nowMs);
  if (!active) return null;
  const Tag = inline ? "span" : "p";
  return <Tag id={id} className={styles.status} data-user-status data-compact={compact || undefined}>
    {active.text}
