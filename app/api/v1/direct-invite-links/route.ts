...
import { createDevDirectInviteLink } from "@/lib/dev-api-store";
...
export async function POST(request: Request) {
  ...
  const result = createDevDirectInviteLink(await devSessionToken(), body.token, key);
  return result.kind === "ok"
    ? NextResponse.json(result.value, { status: result.value.replayed ? 200 : 201, headers: NO_STORE_HEADERS })
