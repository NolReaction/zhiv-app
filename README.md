...
**Версия:** 0.5.3 · **Стек:** React 19, Next.js 16, Kotlin/Ktor, PostgreSQL 18, Docker Compose, Caddy.
...
- Серия отметок и личный календарь по нажатию на огонёк; небольшая игра с локальными рекордами.
- Компактные карточки людей и профиль, единое меню добавления, стеклянный календарь с учётом безопасной области телефона.
...
В `curl` замените домен своим. Compose запускает подготовку ролей БД и отдельный процесс миграций перед API; текущая схема включает V19.
