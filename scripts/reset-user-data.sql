...
DECLARE
    expected_tables text[] := ARRAY[
        ...
        'user_status_write_keys', 'user_timezone_write_keys', 'world_commands', 'world_ledger', 'world_profiles'
    ];
    actual_tables text[];
    table_name text;
    remaining bigint;
...
BEGIN
    ...
    END IF;
    IF (SELECT version FROM public.flyway_schema_history ORDER BY installed_rank DESC LIMIT 1) IS DISTINCT FROM '25'
        OR EXISTS (SELECT 1 FROM public.flyway_schema_history WHERE NOT success) THEN
        RAISE EXCEPTION 'Expected successfully applied migration V25. Reset cancelled.';
    END IF;
    ...
    END IF;
    RAISE NOTICE 'All 39 application tables are empty. Migration history is preserved.';
END $$;
COMMIT;
