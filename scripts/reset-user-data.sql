...
DECLARE
    expected_tables text[] := ARRAY[
        ...
        'game_monthly_scores', 'game_profiles', 'game_sessions',
...
BEGIN
    ...
    END IF;
    IF (SELECT version FROM public.flyway_schema_history ORDER BY installed_rank DESC LIMIT 1) IS DISTINCT FROM '20'
        OR EXISTS (SELECT 1 FROM public.flyway_schema_history WHERE NOT success) THEN
        RAISE EXCEPTION 'Expected successfully applied migration V20. Reset cancelled.';
    END IF;
    ...
    END IF;
    RAISE NOTICE 'All 33 application tables are empty. Migration history is preserved.';
END $$;
COMMIT;
