...
BEGIN
    ...
    END IF;
    IF (SELECT version FROM public.flyway_schema_history ORDER BY installed_rank DESC LIMIT 1) IS DISTINCT FROM '28'
        OR EXISTS (SELECT 1 FROM public.flyway_schema_history WHERE NOT success) THEN
        RAISE EXCEPTION 'Expected successfully applied migration V28. Reset cancelled.';
    END IF;
    ...
    END IF;
    RAISE NOTICE 'All 42 application tables are empty. Migration history is preserved.';
END $$;
COMMIT;
