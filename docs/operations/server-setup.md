...
Следующие команды предназначены для первого запуска с подготовленными настройками и секретами. Для существующего сервера используйте процесс обновления ниже.
...
В `curl` замените домен своим. Compose запускает подготовку ролей БД и отдельный процесс миграций перед API; текущая схема ветки включает V27 (модерация и анализ кликов; предыдущие миграции — мир, диагностика и синхронизация).
...
Дождитесь полного CI для выпуска, переключитесь на `master`, получите проверенный коммит и запустите `bash scripts/deploy-update.sh`. Скрипт включает резервную копию и принудительное применение конфигурации Caddy. Подробности: [эксплуатация](operations.md). Сброс БД не нужен.
