...
пользователей, обращения с журналом обработки, историю, связи, приглашения, сеансы, привязки способов входа,
...
`reset-user-data.sh` очищает только данные текущей проверенной схемы и сохраняет
миграции. Для полностью новой базы используется именно `reset-database.sh`.

## Подготовить проверенную версию

Используйте проверенный коммит текущего выпуска и чистый checkout. Обычное обновление выполняется по [инструкции эксплуатации](operations.md); полный сброс для обновления не нужен. Убедитесь, что известен способ восстановления резервной копии.
...
таблиц пользовательских данных и запускает сервисы с ожиданием healthcheck. История Flyway
будет новой и заполненной выполненными миграциями. Служебная `game_tap_collection_metadata`
должна содержать ровно одну метку, соответствующую выполнению V28 в новой базе;
её корректность проверяется отдельно.
