...
Все семь редактируемых PNG находятся в `art/world/prototype/`: `forest-ground`, `home-level-1/2/3`, `workshop-empty`, `workshop-level-1/2`. Каждый файл уже содержит готовый участок. Экспортируйте PNG целиком в одноимённый WebP в `public/world/prototype/`, затем выполните `npm run world:export` и `npm run world:check`. ZIP и дополнительная нарезка не нужны. [Каталог и порядок редактирования](../../art/README.md).
...
Текущие изображения зданий — непрозрачные фрагменты участка с согласованной кромкой. Они подготовлены под существующее место на карте. Для переноса здания нужно вручную согласовать землю по краям его PNG с новым окружением и при необходимости изменить `forest-ground.png`. Перемещение в Tiled само по себе эту художественную работу не выполняет.
...
Первый уровень дома уже подготовлен в `art/world/prototype/home-level-1.png`, пустое место мастерской — в `art/world/prototype/workshop-empty.png`. Для изменения этих состояний редактируйте соответствующий готовый PNG.
...
Первоначальная версия прототипа, до переименования файлов и упрощения работы с PNG, проверена в Tiled 1.12.2: проект и карта открылись в редакторе, семь изображений прочитались, нативный экспорт и растеризация завершились успешно. Повторно экспортированная карта после обработки нашим компилятором дала полностью идентичную сцену: мир 1254 × 1254, два места зданий, шесть состояний и маршрут `home-walk`. Фоновая текстура имеет размер 1254 × 1254.
