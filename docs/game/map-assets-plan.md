...
> Исторический документ о прежней сцене. Текущий Tiled-мир: [устройство и точки изменения](world-foundation.md), [редактор](tiled-editor.md), [свет](world-lighting.md), [погода и живность](world-atmosphere.md). Указанные ниже координаты, доступные механики и файлы рендера могут относиться к старой версии.
...
Один вызов встроенного imagegen, без повторных вариантов. В качестве референса использована присланная пользователем карта. Запрошено 3072 × 3072 либо максимальное поддерживаемое разрешение; фактически получен PNG 1254 × 1254. Файл сохранён без дополнительной обработки в `art/world/main/forest-map.png`. Для домашней поляны теперь добавлен отдельно прорисованный детальный участок `art/world/main/home-clearing.png` с сохранением масштаба и проверкой привязок. Сам обзорный файл региона не растягивался и не заменялся.
