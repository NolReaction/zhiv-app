...
| Изменить проходимость и поиск пути | [`navigation.ts`](../features/world/navigation.ts); `WalkAreas`, `Obstacles`, `PointsOfInterest` в [`forest.tmj`](../world/tiled/forest.tmj) | [Свободная полянка](game/tiled-editor.md#свободная-полянка-и-места-занятий) |
...
| Изменить исполнение ходьбы, прятки, пробуждение и сон | [`clearing-activity.ts`](../features/world/clearing-activity.ts), специальные подходы `Routes` в `world/tiled/forest.tmj` | [Маршруты Tiled](game/tiled-editor.md#короткие-прогулки-возле-дома) |
