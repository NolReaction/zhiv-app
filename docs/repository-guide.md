...
| Бабочек, светлячков, птиц и погоду новой карты | `features/world/forest-atmosphere.ts`; птицы и точки посадки — `forest-birds.ts`, капли и всплески — `forest-rain.ts`, лужи — `forest-ground-weather.ts`, река — `forest-water.ts`, границы — `Water`/`WaterExclusions` в Tiled; часы и паузы — `new-map-scene.ts` |
