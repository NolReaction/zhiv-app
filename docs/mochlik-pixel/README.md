...
- `lib/mochlik/habitat.ts` — движения, рост, сон и очередь действий.
- `lib/mochlik/feeding.ts` — общие фазы еды и момента поглощения.
- `lib/mochlik/lighting.ts` — день и сумерки в часовом поясе профиля.
- `lib/mochlik/pixel-frame.ts` — поза, направление, выглядывание.
- `lib/mochlik/pixel-sprite.ts` — редактируемый спрайт 48×48 и позы лап/лица.
- `lib/mochlik/scene.ts` — Canvas 256×256, листва, удерживаемый гриб, свет и буквы сна.
- `public/mochlik-pixel/forest.webp` — лесная карта.
...
Полный список изменений и порядок обновления: [релиз 0.6.0](../release-0.6.0.md).
