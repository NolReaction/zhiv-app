...
В production локальный API всегда заблокирован: переменная `ENABLE_DEV_API` больше не открывает имитацию. Memory API работает только в режимах `development`/`test`; реальные запросы `/api` обслуживает Ktor. [db/schema.ts](../db/schema.ts) — пустая заготовка Sites/D1; схема настоящего приложения находится в Flyway-миграциях `apps/api/`.
