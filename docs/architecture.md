> Актуальная версия — 0.5.0: [вход через ВК и почту](auth-0.5.0.md), [изменения](release-0.5.0.md), [развёртывание](../README.md). Ниже сохранены исходные проектные решения; разделы восстановления через друзей исторические и не описывают действующий резервный код.
